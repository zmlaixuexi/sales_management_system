...
| 测试总计 | 438 |
| 后端覆盖率 | ~94% |
| 覆盖模块 | 认证、商品、客户、订单、库存、收款、报表、审计日志、数据导出、批量导入、权限校验、速率限制、SQL 注入防护、请求 ID 中间件、CORS 验证、日志格式器、金额计算 |
...
| security | 认证/权限/速率限制 | 33 |
| export | 导出功能 | 18 |
| import | 导入功能 | 18 |
| report | 报表和审计日志 | 38 |
| integration | 集成测试 | 33 |
| infra | 基础设施（健康检查/中间件/日志） | 17 |
...
### test_order_crud.py（21 个测试）

订单 CRUD + 状态流转全生命周期：创建（正常/空明细/客户不存在/商品不存在/零数量/负价拒绝）、详情/404/列表/状态筛选、编辑草稿（修改明细+金额重算+负价拒绝）、确认（库存扣减验证）、取消（库存回滚验证）、库存不足确认失败。

### test_payment_crud.py（11 个测试）

收款登记 + 冲正：创建（部分收款→partially_paid、全额→completed）、超额收款、零金额、草稿不可收款、订单不存在、列表全量/按 order_id 筛选、冲正/重复冲正/不存在。
...
权限辅助函数单元测试：`_get_user_permissions` 多角色收集/空角色/跨角色去重，`has_permission` 超级用户/有权限/无权限。
...
| ErrorBoundary.test.tsx | 3 | 正常渲染 + 错误捕获 + 重试恢复 |
...
| ProtectedRoute.test.tsx | 4 | 无 token 重定向/加载中 Spin/已认证渲染子组件/fetchUser 失败 |
