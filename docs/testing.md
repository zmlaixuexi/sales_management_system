...
### test_sanitize.py（23 个测试）

`escape_like()` 函数特殊字符转义（%、_、\\）、`strip_html()` XSS 防护（script 标签/事件属性/style 标签/嵌套标签/正常 HTML 保留）、schema 字段消毒（商品名称/客户名称/客户邮箱/用户邮箱/商品备注/客户联系人/客户备注/用户名/用户显示名/订单备注/收款备注）。
