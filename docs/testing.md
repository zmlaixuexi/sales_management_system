...
| 测试总计 | 1747 |
| 后端覆盖率 | 100.00% |
| 前端覆盖页面 | Products（搜索/筛选/空状态/错误/加载/状态标签/删除确认/导入导出）、ProductForm（名称 maxLength/必填字段/高级设置切换/图片上传/SKU 不可见）、CustomerForm（字段 maxLength/来源/等级/跟进状态选项/编辑模式数据填充）、OrderForm（客户选择/行数统计/合计金额/备注限制/编辑模式数据加载/保存按钮）、Dashboard（仪表盘统计）、Inventory（库存管理）、OrderDetail（订单详情）、ReportsCenter（报表中心）、Users（用户管理）、AuditLogs（审计日志）、ErrorBoundary（错误边界）、auth-store（认证状态）、API 模块（认证/商品/客户/订单/库存/用户/报表）、工具函数（formatAmount/formatPercent/downloadCsv/request/usePaginatedList/useSubmit/statusMaps/client-interceptor） |
...
|---|---|---|
| crud | CRUD 操作测试 | 356 |
| boundary | 边界值和异常路径 | 207 |
| security | 认证/权限/速率限制/XSS 防护 | 153 |
| export | 导出功能 | 75 |
| import | 导入功能 | 46 |
| report | 报表和审计日志 | 198 |
| integration | 集成测试 | 53 |
...
文件上传校验单元测试：扩展名白名单、MIME 类型、文件大小、正常通过、webp 格式、大写扩展名、扩展名与 MIME 独立校验、边界大小（恰好等于限制/超限 1 字节）、魔数字节校验（JPEG/PNG 有效头、无效头、空文件）。所有校验异常统一为 HTTPException（与 service 层金标准对齐）。
