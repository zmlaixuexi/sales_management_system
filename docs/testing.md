...
| client-interceptor.test.ts | 18 | 401 刷新、403/404/500 错误消息、网络错误、重试保护、429 重试（含完整重试路径覆盖）、_toastDisplayed 标记、error.message 提取（新格式）、旧格式兼容、409 业务错误、401 刷新失败清除 token、非 401 data.message fallback |
...
| Products.test.tsx | 26 | 搜索/筛选器渲染、新增按钮、商品数据表格、SKU/名称显示、停用按钮条件显示、状态标签、表格列字段、导出按钮调用 downloadCsv、删除确认点击、停用按钮点击、删除失败不崩溃、停用失败不崩溃、状态筛选器选项、分类空值、无图片显示 |
| Customers.test.tsx | 19 | 搜索/来源筛选器渲染、新增按钮、客户数据表格、名称/联系人/电话显示、来源中文映射、等级标签、空值占位符、表格列字段、删除确认点击、导出按钮调用 downloadCsv、导入按钮存在、编辑按钮存在、删除失败不崩溃、来源筛选器选项、筛选空数据、未知来源 |
