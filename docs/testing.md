...
### test_audit_log.py（60 个测试）

审计日志验证。每个测试先执行业务操作，再查询审计日志确认记录正确。覆盖：登录成功/失败、商品 CRUD（创建/编辑/停用）、客户 CRUD（创建/编辑/转移/软删除）、订单操作（创建/确认/取消）、收款登记/冲正、库存调整、用户创建/编辑。字段完整性验证（before_data/after_data 含正确变更前后值）、权限 403（无 audit:log）、未认证 401（列表/操作类型）、无效参数 422、LIKE 注入防护、分页边界。横断完整性验证（所有日志包含 actor_name/ip_address/action/resource_type/created_at，login_failed 的 actor_id 可为 None）。客户删除含订单保护（返回 400）。收款冲正 before_data/after_data 含 order_id。搜索/过滤结果完整性验证（日期范围筛选结果均在范围内、关键字搜索按 actor_name 匹配、不匹配关键字返回空、组合 action+日期筛选、resource_id 部分匹配）。库存调整 before_data 含 name 和 stock_quantity。API 响应字段完整性（user_agent/request_id 非空、resource_id 有效 UUID）。创建操作 before_data 为 None 验证。订单创建 after_data 含 order_no/total_amount。created_at 降序排列验证。
...
### test_reports_audit.py（60 个测试）

销售汇总（6 种 period）、趋势、排行、预警、审计日志查询/筛选/权限、客户排行（含数据范围过滤和利润可见性）、销售人员排行（含数据范围过滤和利润可见性）、未认证 401（6 端点 parametrize）、排行 limit 边界值 422、库存预警负数阈值 422、无效 period 错误码验证、销售汇总/趋势/排行零数据验证（无订单用户零值/空列表/日期填充）、库存预警权限 403 和结构验证。
