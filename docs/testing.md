...
### test_health.py（5 个测试）

健康检查、版本接口和中间件验证，无需认证。
...
### test_boundary.py（36 个测试）

认证边界、订单状态机、收款边界（草稿收款、超额、冲正回退）、用户管理、库存调整。

### test_reports_audit.py（22 个测试）

销售汇总（6 种 period）、趋势、排行、预警、审计日志查询/筛选/权限。

### test_product_import.py（8 个测试）

商品 CSV 批量导入：成功、SKU 自动生成、重复 SKU、空名称、非 CSV、认证、中文表头。

### test_customer_import.py（8 个测试）

客户 CSV 批量导入：成功、手机号重复、批量内去重、空名称、非 CSV、认证。
