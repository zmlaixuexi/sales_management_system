...
### test_audit_log.py（45 个测试）

审计日志验证。每个测试先执行业务操作，再查询审计日志确认记录正确。覆盖：登录成功/失败、商品 CRUD（创建/编辑/停用）、客户 CRUD（创建/编辑/转移）、订单操作（创建/确认/取消）、收款登记/冲正、库存调整、用户创建/编辑。新增字段完整性验证（before_data/after_data 含正确值）、权限 403（无 audit:log）、未认证 401（列表/操作类型）、无效参数 422、LIKE 注入防护、分页边界。
