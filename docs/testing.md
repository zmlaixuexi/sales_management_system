...
### test_order_crud.py（46 个测试）

订单 CRUD + 状态流转全生命周期：创建（正常/空明细/客户不存在/商品不存在/零数量/负价拒绝/低于成本价拒绝）、详情/404/列表/状态筛选/客户筛选、编辑草稿（修改明细+金额重算+负价拒绝/低于成本价拒绝）、确认（库存扣减验证）、取消（库存回滚/商品已删除跳过）、库存不足确认失败、低于成本价阻止下单、订单号后缀回退、零利润边界（unit_price=cost_price）、订单操作日志端点、取消草稿不产生库存变动。
