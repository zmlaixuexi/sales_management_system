...
| 测试文件 | 测试数 | 覆盖内容 |
|---|---|---|
| Dashboard.test.tsx | 14 | 标题/期间选择器渲染、loading 状态、统计卡片显示、API 调用参数、数值正确性、表格数据、API 失败错误提示、期间切换、期间选项标签、排行数据渲染、趋势空状态 |
| Products.test.tsx | 15 | 搜索/筛选器渲染、新增按钮、商品数据表格、SKU/名称显示、停用按钮条件显示、状态标签、表格列字段、导出按钮调用 downloadCsv、删除确认点击、停用按钮点击 |
| Customers.test.tsx | 15 | 搜索/来源筛选器渲染、新增按钮、客户数据表格、名称/联系人/电话显示、来源中文映射、等级标签、空值占位符、表格列字段、删除确认点击、导出按钮调用 downloadCsv、导入按钮存在、编辑按钮存在 |
| Orders.test.tsx | 14 | 搜索/状态筛选器渲染、新建订单按钮、订单数据表格、订单号/明细数显示、状态标签、金额货币符号、空创建时间占位符、表格列字段、导出按钮点击调用 downloadCsv、详情按钮存在、订单号可点击跳转 |
| AuditLogs.test.tsx | 15 | 页面标题渲染、筛选器、审计表格、操作标签中文映射、资源类型中文映射、空值占位符、表格列字段、fetchAuditActions 挂载调用、操作类型选项填充、资源类型选项填充、操作类型筛选重置分页、资源类型筛选重置分页 |
| OrderDetail.test.tsx | 19 | 订单号显示、返回按钮、草稿编辑/确认按钮、状态标签、明细表格、收款记录、fetchOrder ID 调用、加载失败提示、操作日志卡片渲染、日志数据显示、日志操作类型、日志加载失败不阻塞、fetchOrderLogs ID 调用、确认订单交互（confirmOrder）、取消订单交互（cancelOrder）、冲正按钮确认弹窗、确认失败错误提示、已确认状态显示登记收款按钮 |
| ProductForm.test.tsx | 20 | 新增标题、返回按钮、表单字段、必填校验、提交/取消按钮、图片上传、新增模式不加载商品、返回导航、取消导航 |
...
| CustomerDetail.test.tsx | 11 | 客户名称、返回按钮、编辑/删除按钮、等级标签、关联订单表格、fetchCustomer ID 调用、fetchOrders customer_id 筛选、加载失败提示、确认删除调用 deleteCustomer、编辑按钮跳转编辑页 |
| ReportsCenter.test.tsx | 14 | 页面标题和周期选择器、五个标签页（销售概览/商品排行/客户排行/销售排行/库存预警）、统计卡片、趋势表格、商品排行数据、API 默认周期 30d 调用、库存预警标签、概览加载失败提示、周期选项标签、商品排行空状态、客户排行错误提示 |
| Payments.test.tsx | 15 | 页面标题、总记录数、收款数据行、金额格式化、收款方式中文、状态标签（正常/已冲正）、订单 ID 跳转、空状态、导出按钮渲染、导出按钮调用 downloadCsv、订单 ID 点击跳转详情 |
| Users.test.tsx | 16 | 用户数据行、用户名和显示名、角色标签、超级管理员标记、新建按钮、编辑按钮、空值占位符、新建弹窗打开、fetchRoles 挂载调用、角色选择器选项填充 |
