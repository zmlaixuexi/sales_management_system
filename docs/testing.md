...
### test_audit_log.py（49 个测试）

审计日志验证。每个测试先执行业务操作，再查询审计日志确认记录正确。覆盖：登录成功/失败、商品 CRUD（创建/编辑/停用）、客户 CRUD（创建/编辑/转移/软删除）、订单操作（创建/确认/取消）、收款登记/冲正、库存调整、用户创建/编辑。字段完整性验证（before_data/after_data 含正确变更前后值）、权限 403（无 audit:log）、未认证 401（列表/操作类型）、无效参数 422、LIKE 注入防护、分页边界。横断完整性验证（所有日志包含 actor_name/ip_address/action/resource_type/created_at，login_failed 的 actor_id 可为 None）。客户删除含订单保护（返回 400）。收款冲正 before_data/after_data 含 order_id。
