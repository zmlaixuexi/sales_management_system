...
### test_reports_audit.py（67 个测试）

销售汇总（6 种 period）、趋势、排行、预警、审计日志查询/筛选/权限、客户排行（含数据范围过滤和利润可见性）、销售人员排行（含数据范围过滤和利润可见性）、未认证 401（6 端点 parametrize）、排行 limit 边界值 422、库存预警负数阈值 422、无效 period 错误码验证、销售汇总/趋势/排行零数据验证（无订单用户零值/空列表/日期填充）、库存预警权限 403 和结构验证、period=custom 返回 400、审计日志不存在 action/resource_type 返回空列表、趋势日期范围验证、排行默认 limit 和结构验证。
