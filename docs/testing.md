...
| test_audit_service.py | （无） | 纯函数测试，审计脱敏/模型转换/log_action user_agent 截断 |
| test_logging.py | （无） | 纯函数测试，JSON/文本日志格式器+setup_logging |
| test_schema_validators.py | （无） | 纯函数测试，Pydantic field_validator |
| test_config_validation.py | （无） | 配置值域校验，JWT/DB_POOL/SIZE/ROWS/RATE_LIMIT 边界 |
| test_business_metrics.py | （无） | Prometheus 自定义业务指标存在性/标签/计数器递增/Gauge 设值/注册表校验 |
...
| test_file_service.py | （无） | 文件服务单元测试（扩展名/MIME/魔数字节/大小限制/webp AND 验证） |
| test_export_helpers.py | （无） | 导出辅助函数测试 |
...
|---|---|---|
| crud | CRUD 操作测试 | 358 |
| boundary | 边界值和异常路径 | 209 |
| security | 认证/权限/速率限制/XSS 防护 | 155 |
| export | 导出功能 | 77 |
| import | 导入功能 | 48 |
| report | 报表和审计日志 | 200 |
| integration | 集成测试 | 55 |
| infra | 基础设施（健康检查/中间件/日志/Prometheus 指标） | 45 |

---

## 后端测试文件详解

### test_health.py（27 个测试）

健康检查、版本接口、中间件验证、异常处理、CORS、生产环境安全检查、Prometheus /metrics 端点验证，无需认证。
...
| test_31_expired_refresh_token_rejected | 已过期 refresh token 返回 401 |
| test_32_refresh_token_wrong_signature_rejected | 错误签名 refresh token 返回 401 |
| test_33_refresh_token_nonexistent_user_rejected | 不存在用户 refresh token 返回 401 |
| test_34_refresh_token_missing_type_rejected | 缺少 type 字段 refresh token 返回 401 |
| test_35_disabled_user_access_token_immediately_invalid | 禁用用户 access token 立即失效 |
| test_36_disabled_user_refresh_token_immediately_invalid | 禁用用户 refresh token 立即失效 |
| test_37_password_change_old_access_token_still_works | 密码修改后旧 access token 仍可用（无状态 JWT） |
| test_38_password_change_old_refresh_token_still_works | 密码修改后旧 refresh token 仍可用（无状态 JWT） |
...
### test_boundary.py（129 个测试） | test_sanitize.py（23 个测试）

认证边界、订单状态机、收款边界（草稿收款、超额、冲正回退）、用户管理、库存调整、流水类型筛选、订单创建/编辑备注超长 422 边界、收款备注超长 422 边界。

### test_reports_audit.py（70 个测试）

销售汇总（6 种 period）、趋势、排行、预警、审计日志查询/筛选/权限、客户排行（含数据范围过滤和利润可见性）、销售人员排行（含数据范围过滤和利润可见性）、未认证 401（6 端点 parametrize）、排行 limit 边界值 422、库存预警负数阈值 422、无效 period 错误码验证、销售汇总/趋势/排行零数据验证（无订单用户零值/空列表/日期填充）、库存预警权限 403 和结构验证、period=custom 返回 400、period=30d 响应结构验证、审计日志不存在 action/resource_type 返回空列表、趋势无效日期范围验证、商品排行默认 limit≤10、销售排行默认结构验证。
...
### test_sanitize.py（30 个测试）

`escape_like()` 函数特殊字符转义（%、_、\\）、`strip_html()` XSS 防护（script 标签/事件属性/style 标签/嵌套标签/正常 HTML 保留）、`strip_control_chars()` 控制字符清理（null byte/bell/escape 移除、tab/newline/cr 保留）、`sanitize_text()` 组合消毒（HTML + 控制字符）、schema 字段消毒（商品名称/客户名称/客户邮箱/用户邮箱/商品备注/客户联系人/客户备注/用户名/用户显示名/订单备注/收款备注）。

### test_user_management.py（39 个测试）

用户管理 CRUD + 角色列表 + 边界验证：列表/搜索、创建（含重复用户名）、编辑、禁用切换、角色变更、编辑时无效角色 ID 返回 400、403 权限校验、分页参数、未认证 401、弱密码 422、创建时无效角色 ID 400、角色列表 GET /users/roles（正常返回/非管理员 403/未认证 401）、管理员不能停用自身账号、非管理员查看用户列表 403、编辑用户无效 UUID 400、创建用户畸形 role_ids 400、编辑用户空角色列表清空、创建用户密码无数字 422、编辑用户 display_name 超长 422、密码强度边界验证（过短/纯特殊字符/无数字/无字母/修改密码纯特殊字符均返回 422）、LIKE 注入防护、分页边界、修改密码纯特殊字符 422。
...
### test_product_crud.py（47 个测试）

商品 CRUD 成功路径 + CSV 导入 + 库存变更行锁：详情获取、软删除、删除后不可见、列表排除已删除、分类筛选/排序、SKU 更新、CSV 导入（成功/编码错误/空表头/行错误/SKU 重复/大小限制）、编辑库存变更记录 InventoryMovement（quantity_before/change/after 正确性）、库存不变不产生流水、不涉及库存编辑走普通查询路径。

### test_order_crud.py（57 个测试）

订单 CRUD + 状态流转全生命周期：创建（正常/空明细/客户不存在/商品不存在/零数量/负价拒绝/低于成本价拒绝/零价阻止下单）、详情/404/列表/状态筛选/客户筛选/关键字搜索（LIKE 注入防护）/分页边界/降序排列、编辑草稿（修改明细+金额重算+负价拒绝/低于成本价拒绝/仅更新备注）、确认（库存扣减验证）、取消（库存回滚/商品已删除跳过/无效UUID 422/已完成订单 400）、库存不足确认失败、低于成本价阻止下单、订单号后缀回退、零利润边界（unit_price=cost_price）、订单操作日志端点（含成本字段权限过滤）、取消草稿不产生库存变动、重复确认第二次 400（行锁防护）、取消已确认订单库存正确回滚验证。

### test_payment_crud.py（32 个测试）

收款登记 + 冲正：创建（部分收款→partially_paid、全额→completed）、超额收款、零金额、草稿不可收款、订单不存在、列表全量/按 order_id 筛选/非管理员数据范围过滤/分页、冲正/重复冲正/不存在/关联订单已删除、已取消订单收款拒绝、已完成订单收款拒绝、负数金额、无权限用户收款/冲正 403、冲正后 completed→partially_paid 回退、冲正全部金额后→confirmed 回退、非订单所有者收款拒绝、收款列表排除已冲正记录、金额多小数位精度处理、同一笔收款冲正两次第二次返回 404（行锁防护）。路径已对齐规范文档 POST /sales-orders/{id}/payments。

### test_inventory_crud.py（27 个测试）

库存调整 + 流水查询：手工调整（增加/减少/归零）、零调整拒绝、超量扣减拒绝、商品不存在、流水列表/按 product_id 筛选/按 movement_type 筛选/组合筛选（product_id + movement_type）、字段完整性校验、无调整权限 403、无列表权限 403、已删除商品调整 404、流水分页、order_confirm 类型筛选、未认证访问 401（流水/调整）、无效商品 UUID 400、调整响应字段验证、不存在 product_id 筛选空列表、备注 XSS strip_html、小数数量拒绝、备注 500/501 字符长度边界、库存调整审计日志 before_data/after_data 字段完整性验证。
...
### test_file_service.py（20 个测试）

文件上传校验单元测试：扩展名白名单、MIME 类型、文件大小、正常通过、webp 格式、大写扩展名、扩展名与 MIME 独立校验、边界大小（恰好等于限制/超限 1 字节）、魔数字节校验（JPEG/PNG 有效头、无效头、空文件）、webp AND 魔数验证（非 WebP RIFF 拒绝、合法 WebP 通过、仅 WEBP 无 RIFF 拒绝）。孤立文件清理（cleanup_orphan_files）：超过 24h 未绑定文件删除、已绑定文件保留、未超时文件保留、批量清理计数。
...
### test_audit_service.py（13 个测试）

审计服务内部函数测试：`_mask_sensitive` None/空字典/密码脱敏/token 脱敏/手机号脱敏/邮箱脱敏/无匹配，`model_to_dict` UUID 转字符串/None 跳过。`log_action` 超长 user_agent 截断 500 字符/短 user_agent 不变。
...
### test_payment_register.py（13 个测试）

收款登记服务测试（SQLite 内存，fixture 隔离）：`register_payment` 已确认订单部分收款→部分收款状态、全额收款→已完成、分两次收款→订单完成、订单不存在(404)、草稿/已取消/已完成订单不可收款(400)、超额收款拒绝(400)、恰好等于剩余通过、操作人和更新人记录、并发防护 inflight 标记拦截(429)、不同订单互不干扰、失败后 inflight 标记清除可重试。
...
| ErrorBoundary.test.tsx | 6 | 正常渲染 + 错误捕获 + 重试恢复 + 路由变化重置 + 返回首页 + 无 message 默认提示 |
...
| useDocumentTitle.test.ts | 4 | 标题格式设置/默认标题/卸载恢复/标题变化更新 |
| client-interceptor.test.ts | 19 | 401 刷新、403/404/500 错误消息、网络错误、重试保护、429 重试（含完整重试路径覆盖、无 retry-after 头默认等待）、_toastDisplayed 标记、error.message 提取（新格式）、旧格式兼容、409 业务错误、401 刷新失败清除 token、非 401 data.message fallback |
...
| orders-api.test.ts | 16 | fetchOrders（关键词/客户ID/分页/无参数）/fetchOrder（完整明细+收款）/create（多商品）/update（明细）/confirm/cancel/fetchOrderLogs（无参数/分页参数） |
...
| 测试文件 | 测试数 | 覆盖内容 |
|---|---|---|
| Dashboard.test.tsx | 18 | 标题/期间选择器渲染、loading 状态、统计卡片显示、API 调用参数、数值正确性、表格数据、API 失败错误提示、期间切换、期间选项标签、排行数据渲染、趋势空状态、无预警空数据、无排行空数据、零库存红色标签、部分 API 失败 |
| Products.test.tsx | 48 | 搜索/筛选器渲染、新增按钮、商品数据表格、SKU/名称显示、停用按钮条件显示、状态标签、表格列字段、导出按钮调用 downloadCsv、删除确认点击、停用按钮点击、删除失败不崩溃、停用失败不崩溃、状态筛选器选项、分类空值、无图片显示、导入（CSV API 调用/成功刷新/success=false 不刷新/API 错误/文件为空/按钮触发文件选择器/卸载时 ref null 清理）、成本权限列显示/隐藏、未知状态、翻页、ToastDisplayed 静默 |
| Customers.test.tsx | 37 | 搜索/来源筛选器渲染、新增按钮、客户数据表格、名称/联系人/电话显示、来源中文映射、等级标签、空值占位符、表格列字段、删除确认点击、导出按钮调用 downloadCsv、导入按钮存在、编辑按钮存在、删除失败不崩溃、来源筛选器选项、筛选空数据、未知来源、导入（成功刷新/success=false 不刷新/文件为空/按钮触发文件选择器/卸载时 ref null 清理）、ToastDisplayed 静默、未知等级、翻页 |
| Orders.test.tsx | 19 | 搜索/状态筛选器渲染、新建订单按钮、订单数据表格、订单号/明细数显示、状态标签、金额货币符号、空创建时间占位符、表格列字段、导出按钮点击调用 downloadCsv、详情按钮存在、订单号可点击跳转、未知状态原始值、状态筛选器选项、筛选空数据、毛利列显示、搜索框 placeholder |
| AuditLogs.test.tsx | 18 | 页面标题渲染、筛选器、审计表格、操作标签中文映射、资源类型中文映射、空值占位符、表格列字段、fetchAuditActions 挂载调用、操作类型选项填充、资源类型选项填充、操作类型筛选重置分页、资源类型筛选重置分页、未知操作类型、未知资源类型、fetchAuditActions 失败 |
| OrderDetail.test.tsx | 22 | 订单号显示、返回按钮、草稿编辑/确认按钮、状态标签、明细表格、收款记录、fetchOrder ID 调用、加载失败提示、操作日志卡片渲染、日志数据显示、日志操作类型、日志加载失败不阻塞、fetchOrderLogs ID 调用、确认订单交互（confirmOrder）、取消订单交互（cancelOrder）、冲正按钮确认弹窗、确认失败错误提示、已确认状态显示登记收款按钮、取消失败错误提示、空明细空状态、空日志空状态 |
| ProductForm.test.tsx | 23 | 新增标题、返回按钮、表单字段、必填校验、提交/取消按钮、图片上传、新增模式不加载商品、返回导航、取消导航、编辑模式加载失败验证、提交按钮类型、商品图片字段、名称 maxLength |
| CustomerForm.test.tsx | 21 | 新增标题、返回按钮、表单字段、必填校验、提交/取消按钮、来源/等级/跟进状态下拉、新增模式不加载客户、返回导航、取消导航 |
| OrderForm.test.tsx | 20 | 新建标题、返回按钮、客户选择字段、添加商品按钮、创建订单/取消按钮、备注字段、新建模式不加载订单、返回导航、取消导航 |
| CustomerDetail.test.tsx | 15 | 客户名称、返回按钮、编辑/删除按钮、等级标签、关联订单表格、fetchCustomer ID 调用、fetchOrders customer_id 筛选、加载失败提示、确认删除调用 deleteCustomer、编辑按钮跳转编辑页、删除失败不崩溃、无关联订单、来源中文映射、订单加载失败不阻塞 |
| ReportsCenter.test.tsx | 17 | 页面标题和周期选择器、五个标签页（销售概览/商品排行/客户排行/销售排行/库存预警）、统计卡片、趋势表格、商品排行数据、API 默认周期 30d 调用、库存预警标签、概览加载失败提示、周期选项标签、商品排行空状态、客户排行错误提示、销售排行失败、趋势空数据、库存预警空数据 |
| Payments.test.tsx | 18 | 页面标题、总记录数、收款数据行、金额格式化、收款方式中文、状态标签（正常/已冲正）、订单 ID 跳转、空状态、导出按钮渲染、导出按钮调用 downloadCsv、订单 ID 点击跳转详情、未知收款方式、未知状态、分页信息 |
| Users.test.tsx | 19 | 用户数据行、用户名和显示名、角色标签、超级管理员标记、新建按钮、编辑按钮、空值占位符、新建弹窗打开、fetchRoles 挂载调用、角色选择器选项填充、fetchRoles 错误不阻塞、切换失败不崩溃、弹窗关闭按钮 |
| Inventory.test.tsx | 15 | 页面标题、总记录数、库存变动行、变动类型标签（订单扣减/手动调整）、正数绿色 + 号、负数红色、关联类型、空状态、未知变动类型、未知关联类型、变动量零、空关联类型 |
| Roles.test.tsx | 19 | 渲染标题、新建按钮、挂载加载、表格、模态框字段（标识/显示名/说明/权限）、权限复选框、空状态、错误后空列表、输入框、textarea、编辑模式、创建刷新、删除按钮禁用（有用户关联）、确认删除、保存按钮、fetchPermissions 错误、删除失败、模态框关闭 |
