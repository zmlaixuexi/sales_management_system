...
- 新增 `backend/tests/test_frontend_form_schema_alignment.py`
- 验证必填字段对齐：商品名称、客户名称、订单客户 ID、用户名/密码、登录字段
- 验证字符串长度对齐：商品名称/客户电话/客户邮箱/用户名/密码 maxLength 与 max_length 一致
- 验证数值边界对齐：价格 min=0、库存 ge=0、sort_weight 对称边界、quantity gt=0、库存调整对称边界
- 验证格式验证对齐：手机号正则 1[3-9]\d{9}、邮箱 type:'email'、密码复杂度（字母+数字）
- 验证表单字段覆盖：商品/客户/订单/用户/角色表单覆盖后端 Create Schema 关键字段
- 后端测试总计：5713 项（新增 25 项）
...
- 新增 `backend/tests/test_error_code_uniqueness.py`
- 验证错误码命名规范：UPPER_SNAKE_CASE、无连续下划线、最小长度
- 验证错误码分类归属：AUTH_、RESOURCE_、VALIDATION_、业务特定前缀
- 验证全局唯一性：共享码消息语义一致、核心码必须存在、高频码为通用型
- 验证消息语言：全中文、无纯英文、VALIDATION_FAILED 消息描述性
- 验证响应结构：code/message 配对、main.py 异常处理器使用标准码
