...
后端覆盖率 99%（23 行未覆盖）。剩余未覆盖：products.py CSV 导入（12 行）、deps get_db 生成器（4 行）、customers CSV 导入（3 行）、orders 取消回滚（2 行）、payments 冲正边界（1 行）、非数字订单号（1 行）。建议继续部署体验或安全加固。
