...
| 模块 | 测试数 | 覆盖内容 |
|---|---|---|
| 认证 | 18 | 登录成功/失败、Token 刷新、当前用户、权限校验、禁用用户刷新被拒、密码修改（含未认证 401/空旧密码 422）、登录失败速率限制（连续失败触发 429） |
...
| 审计日志 | 21 | 全操作类型日志、筛选、操作类型列表、用户管理操作审计日志、LIKE 注入搜索防护（%/） |
...
| 用户管理 | 32 | 用户列表/搜索、创建（含重复用户名）、编辑、禁用、角色变更、编辑时无效角色 ID、403 权限、创建/编辑审计日志记录、输入长度溢出、无效UUID 400、畸形role_ids 400、空角色清空、密码无数字 422、LIKE 注入搜索防护（%/） |
| 商品 CRUD | 48 | 商品详情、软删除、列表排除已删除、分类筛选/排序、SKU 更新、CSV 导入、价格变更、输入长度溢出、成本字段权限过滤（列表+详情）、LIKE 注入搜索防护（%/） |
| 客户 CRUD | 34 | 客户详情、编辑、转移、软删除、来源筛选、关键词搜索、CSV 导入、输入长度溢出、备注 XSS 清理、转移不存在用户 404、邮箱重复边界、LIKE 注入搜索防护 |
| 订单 CRUD | 56 | 创建（正常/空明细/客户不存在/商品不存在/零数量/负价拒绝）、详情/404/列表/状态筛选/客户筛选、编辑草稿（含负价拒绝）、确认/取消/库存不足、软删除商品订单确认/取消回归、订单号回退、零利润边界、订单日志端点、取消草稿无库存变动、取消无效UUID 422、取消已完成订单 400、仅更新备注、零价阻止下单、关键字搜索、LIKE 注入防护、page_size=1 边界、降序排列验证、订单日志成本字段权限过滤、page_size=100 边界 |
| 收款 | 33 | 创建（部分收款→完成、超额、零金额、草稿不可收款、订单不存在）、列表/按订单筛选/数据范围过滤、冲正/重复冲正/不存在/关联订单已删除、冲正状态回退、非所有者拒绝、列表排除已冲正、金额多小数位精度、列表降序排列验证、page_size=100 边界 |
| 库存 | 25 | 手工调整（增加/减少/归零/零调整拒绝/超量拒绝/商品不存在）、流水列表/按商品筛选/按类型筛选/组合筛选/字段完整性、备注 XSS 清理、小数数量拒绝、备注长度边界、page_size=100 边界 |
