...
| 订单 CRUD | 54 | 创建（正常/空明细/客户不存在/商品不存在/零数量/负价拒绝）、详情/404/列表/状态筛选/客户筛选、编辑草稿（含负价拒绝）、确认/取消/库存不足、软删除商品订单确认/取消回归、订单号回退、零利润边界、订单日志端点、取消草稿无库存变动、取消无效UUID 422、取消已完成订单 400、仅更新备注、零价阻止下单、关键字搜索、LIKE 注入防护、page_size=1 边界、降序排列验证 |
