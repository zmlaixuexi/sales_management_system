...
| 模块 | 测试数 | 覆盖内容 |
|---|---|---|
| 认证 | 9 | 登录成功/失败、Token 刷新、当前用户、权限校验、禁用用户刷新被拒 |
| 健康检查 | 12 | 健康状态、版本信息、安全响应头、请求日志记录、请求 ID 生成/透传/日志关联、生产环境 JWT 密钥检查、未处理异常 JSON 响应、CORS 允许/拒绝验证 |
...
| 订单 CRUD | 21 | 创建（正常/空明细/客户不存在/商品不存在/零数量/负价拒绝）、详情/404/列表/状态筛选、编辑草稿（含负价拒绝）、确认（库存扣减）/重复确认/已确认不可编辑、取消（库存回滚）/重复取消、库存不足确认失败 |
| 收款 | 11 | 创建（部分收款→完成、超额、零金额、草稿不可收款、订单不存在）、列表/按订单筛选、冲正/重复冲正/不存在 |
| 库存 | 10 | 手工调整（增加/减少/归零/零调整拒绝/超量拒绝/商品不存在）、流水列表/按商品筛选/按类型筛选/字段完整性 |
| 权限辅助函数 | 6 | _get_user_permissions 多角色收集/去重，has_permission 超级用户/有权限/无权限 |
| 订单金额计算 | 9 | _calc_order_totals 金额/零值/精度，_prepare_item 价格/折扣/快照 |
| 商品利润计算 | 6 | _calc_profit 基本利润/除零保护/亏损/零利润/精度/高毛利率 |
| 审计服务函数 | 7 | _mask_sensitive 脱敏，model_to_dict 模型转换 |
| 日志格式器 | 5 | _JsonFormatter JSON 输出/异常/extra_fields，log_action 容错 |
| **合计** | **325** | |
...
| ErrorBoundary | 3 | 正常渲染 + 错误捕获 + 重试恢复 |
...
| ProtectedRoute | 4 | 无 token 重定向/加载中/已认证渲染/fetchUser 失败 |
| **合计** | **113** | |
