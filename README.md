...
本项目完全由vibe coding实现，glm5.1自动化实现
...
```bash
# 后端测试（当前验收运行：5988 个通过，1 个跳过）
cd backend
source .venv/bin/activate
pytest tests/ -v

# 前端测试（当前验收运行：1719 个通过）
cd frontend
npm test
...
> 当前表格保留模块级覆盖说明；精确测试总数以 `pytest tests/ -q` 的实时输出为准。
...
| 商品 CRUD | 62 | 商品详情、软删除、列表排除已删除、分类筛选/排序、SKU 更新、CSV 导入、价格变更、输入长度溢出、成本字段权限过滤（列表+详情）、LIKE 注入搜索防护（%/）、page_size=100 边界、更新不存在商品库存 404（with_for_update 路径） |
...
| 订单 CRUD | 73 | 创建（正常/空明细/客户不存在/商品不存在/零数量/负价拒绝）、详情/404/列表/状态筛选/客户筛选、编辑草稿（含负价拒绝）、确认/取消/库存不足、软删除商品订单确认/取消回归、订单号回退、零利润边界、订单日志端点、取消草稿无库存变动、取消无效UUID 422、取消已完成订单 400、仅更新备注、零价阻止下单、关键字搜索、LIKE 注入防护、page_size=1 边界、降序排列验证、订单日志成本字段权限过滤、page_size=100 边界、备注 XSS 清理、确认/取消不存在订单 404 |
...
| **合计** | **5988 通过 / 1 跳过** | 以当前全量 pytest 运行结果为准 |
...
| 订单列表 | 19 | 渲染/搜索/筛选/新建按钮/表格数据/空状态/错误状态+重试/loading 状态/导出下载/详情按钮/订单号跳转/未知状态/状态筛选器选项/筛选空数据/毛利列显示/搜索框 |
| 审计日志 | 18 | 渲染/筛选/表格/操作标签中文映射/资源类型中文/空值/列标题/fetchAuditActions 调用/空状态/错误状态+重试/loading 状态/操作类型选项填充/资源类型选项填充/筛选变更重置分页/未知操作类型/未知资源类型/fetchAuditActions 失败 |
| 用户列表 | 19 | 渲染/搜索/表格数据/角色标签/超级管理员标记/新建按钮/空状态/loading 状态/错误重试/编辑弹窗/启用停用切换/fetchRoles 调用/角色选择器选项/fetchRoles 错误/切换失败/弹窗关闭 |
...
| 收款列表 | 18 | 渲染/筛选/表格数据/loading 状态/错误重试/收款 ID 截断/空备注显示/导出按钮/导出下载/订单跳转/未知收款方式/未知状态/分页信息 |
| 库存列表 | 15 | 渲染/筛选/表格数据/loading 状态/错误重试/空备注显示/未知变动类型/未知关联类型/变动量零/空关联类型 |
| 报表中心 | 17 | 渲染标题/周期选择器/五个标签页/客户排行/销售排行数据/周期切换重载/API 默认周期/周期选项标签/商品排行空状态/客户排行错误提示/销售排行失败/趋势空数据/库存预警空数据 |
| 客户详情 | 15 | 加载显示客户名称/返回按钮/loading 状态/删除确认/编辑导航/删除失败不崩溃/无关联订单/来源中文映射/订单加载失败不阻塞 |
| 订单详情 | 22 | 加载显示订单号/返回按钮/loading 状态/确认/取消/冲正操作/收款登记/状态标签/取消失败错误提示/空明细/空操作日志 |
| Dashboard | 18 | 渲染看板标题/期间选择器/数据卡片/趋势汇总/库存预警数据/期间选项数/切换重载/期间选项标签/排行数据/趋势空状态/无预警空数据/无排行空数据/零库存红色标签/部分 API 失败 |
